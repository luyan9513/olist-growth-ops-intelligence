...
P12 不增加新模型，而是把已完成的 P0–P11 整理成一个读者能快速判断价值、演示时能顺畅讲解、每个数字都能回到证据文件的项目版本。

目标读者包括数据、增长、商业和电商运营方向的项目审阅者。默认阅读时间分为三档：
...
P12 的截图只用于项目预览，不能替代可交互 Streamlit。建议固定窗口尺寸并使用以下页面：
...
生成产物默认不提交版本库时，演示数字应通过 README 的完整流程重新生成；不得只凭本指南手工填写。
...
该命令会重新生成统一证据，并检查其是否与当前分析、模型和行动产物一致；还会检查五类行动数量加总、容量对照、内部链接、四张 PNG 的格式和尺寸，并运行项目证据与 10 页看板专项测试。它不是完整模型重训；需要从原始数据全量复现时仍使用 `make verify`。
