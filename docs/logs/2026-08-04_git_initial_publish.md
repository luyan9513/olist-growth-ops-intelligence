...
首次本地提交创建后，Git 提示自动使用了主机名形式的 `.local` 作者邮箱。为避免公开 Git 历史暴露不合适的本机地址，在推送前停止，用户自行配置 GitHub 隐私邮箱并 amend；最终首次提交为 `98a9392`。

用户随后将 `main` 推送到 `origin`。复查结果：本地 `main` 与 `origin/main` 均指向 `98a9392`，工作区干净，远端默认分支为 `main`。

首次推送触发 GitHub Actions run `30891909430`。仓库检查和 dbt parse 通过，但 Python 页面测试有 7 项失败，根因是本机 DuckDB/artifacts 隐式存在。该问题没有被写成发布成功，后续修复详见 `2026-08-04_p14_hosted_ci_data_isolation.md`。

## 7. 当前限制与后续

- GitHub 托管 CI 已真实触发，但首次运行失败；必须以 P14 新 run 变绿作为关闭条件；
- P14 修复提交 `ce76e57` 的 GitHub Actions run `30892648947` 已成功，首次失败已闭环；
